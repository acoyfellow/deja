...
Deja succeeds when a fresh agent continues correctly with less re-reading, fewer repeated commands, fewer stale instructions, and fewer tokens than an equivalent session without memory.
...
- [ ] Baseline-vs-Deja real-session evaluation has enough samples for product claims.
...
- [x] Scoped quality report via library and `deja eval`.
...
- [ ] Run baseline-vs-Deja continuation tasks with fixed prompts and fixtures.
