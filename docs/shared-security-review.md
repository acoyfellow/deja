...
**Deployment paused.** Shared Deja works locally. It is not approved for any deployment until this review resolves.

## Why this matters

Shared Deja stores durable personal/work context and delivers it live to connected clients. A mistake exposes more than a demo endpoint: preferences, project decisions, handoffs, possibly internal work context.
...
Each space reaches a different Durable Object via `idFromName("space:" + space)`. Tests prove one token cannot read another space's events, status, or stream. The legacy `DEJA_SHARED_TOKEN=<token>` setting still maps to one `local` space for compatibility.
...
`SharedDeja` reconnects a few seconds before announced expiry, which re-evaluates credentials and resumes from the contiguous watermark.
