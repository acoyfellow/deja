...
const dirs: string[] = [];
const originalScope = process.env.DEJA_SCOPE;
const originalRepository = process.env.DEJA_REPOSITORY;

afterEach(() => {
  if (originalScope === undefined) delete process.env.DEJA_SCOPE;
  else process.env.DEJA_SCOPE = originalScope;
  if (originalRepository === undefined) delete process.env.DEJA_REPOSITORY;
  else process.env.DEJA_REPOSITORY = originalRepository;
  for (const dir of dirs.splice(0)) rmSync(dir, { recursive: true, force: true });
});

describe("automatic memory context", () => {
  test("explicit scope wins", () => {
    const dir = mkdtempSync(join(tmpdir(), "deja-context-"));
    dirs.push(dir);
    process.env.DEJA_SCOPE = "global";
    process.env.DEJA_REPOSITORY = "personal";
...
function repo(name: string, remote: string): string {
  const parent = mkdtempSync(join(tmpdir(), "deja-context-"));
  dirs.push(parent);
