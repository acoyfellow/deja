...
import { SharedDeja } from "../src/shared-client/index.ts";
...
function setup() {
  ...
  const client = new SharedDeja({ gateway: "http://memory.test", fetch: fetcher, mirror, author: "agent", sessionId: "sess" });
  return { authority, client, mirror };
}

describe("SharedDeja facade", () => {
  ...
  test("connected peer receives streamed write into its local mirror", async () => {
    ...
    const b = new SharedDeja({ gateway: "http://memory.test", fetch: fetcher, mirror: bMirror, author: "b", sessionId: "b" });
  ...
  test("sends bearer token when configured", async () => {
    ...
    const d = new SharedDeja({ gateway: "http://m", token: "secret", fetch: fetcher, mirror });
