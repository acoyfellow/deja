...
export interface SharedDejaOptions {
  gateway: string;
  token?: string;
  author?: string;
  sessionId?: string;
...
export class SharedDeja {
  ...
  constructor(opts: SharedDejaOptions) {
    this.gateway = opts.gateway.replace(/\/$/, "");
    this.author = opts.author ?? process.env.DEJA_AUTHOR ?? "unknown-agent";
    this.token = opts.token ?? process.env.DEJA_SHARED_TOKEN;
    this.sessionId = opts.sessionId ?? process.env.DEJA_SESSION ?? ulid();
  ...
  static async connect(opts: SharedDejaOptions): Promise<SharedDeja> {
    const d = new SharedDeja(opts);
    await d.connect();
    return d;
  }
  ...
  async remember(
    ...
    if (!trimmed) throw new Error("deja.shared.remember: text is empty");
  ...
  async handoff(summary: string, opts: SharedHandoffOptions = {}): Promise<SharedWriteReceipt<SharedHandoffPayload>> {
    ...
    if (!trimmed) throw new Error("deja.shared.handoff: summary is empty");
  ...
  async signal(slipId: string, action: SharedSignalAction): Promise<SharedWriteReceipt<SharedSignalPayload>> {
    if (!slipId.trim()) throw new Error("deja.shared.signal: slipId is empty");
  ...
  async delete(slipId: string): Promise<SharedWriteReceipt<SharedDeletePayload>> {
    if (!slipId.trim()) throw new Error("deja.shared.delete: slipId is empty");
