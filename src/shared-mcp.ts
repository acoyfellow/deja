#!/usr/bin/env bun
/** Shared Deja MCP server — remote authority + local live mirror. */
import { Server } from "@modelcontextprotocol/sdk/server/index.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { CallToolRequestSchema, ListToolsRequestSchema } from "@modelcontextprotocol/sdk/types.js";
import { SharedDeja } from "./shared-client/index.ts";
...
export async function runSharedServer(client: SharedDispatchClient): Promise<void> {
  const server = new Server({ name: "deja-shared", version: `${VERSION}-preview` }, { capabilities: { tools: {} } });
...
if (import.meta.main) {
  ...
  if (!gateway) throw new Error("DEJA_SHARED_GATEWAY is required to run shared MCP");
  const client = await SharedDeja.connect({ gateway });
