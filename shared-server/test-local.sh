...
DEJA_SHARED_TOKENS=dev-token:demo
VARS
PORT="${PORT:-8790}"
...
export DEJA_SHARED_GATEWAY="$BASE" DEJA_SHARED_TOKEN="dev-token"
export DEJA_SHARED_MIRROR_DB="$PWD/shared-server/.tmp/a.sqlite"
...
export DEJA_SHARED_MIRROR_DB="$PWD/shared-server/.tmp/b.sqlite"
...
COUNT="$(bun -e 'import { Database } from "bun:sqlite"; const db = new Database(process.argv[1]); console.log(db.query("SELECT COUNT(*) AS n FROM mirror_slips WHERE slip_id = ?").get(process.argv[2]).n); db.close()' "$DEJA_SHARED_MIRROR_DB" "$ID")"
