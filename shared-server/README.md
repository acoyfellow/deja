...
This is the hosted half of shared Deja.
...
DEJA_SHARED_TOKENS=dev-token:personal,team-token:project-docs
EOF
cd shared-server
...
```bash
DEJA_SHARED_GATEWAY=http://127.0.0.1:8790 \
DEJA_SHARED_TOKEN=dev-token \
...
```text
DEJA_SHARED_TOKENS=<token>:<memory-space>,<token>:<memory-space>
```
...
`DEJA_SHARED_TOKEN=<token>` remains temporarily compatible with earlier local
