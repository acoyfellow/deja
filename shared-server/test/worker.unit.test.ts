...
function makeEnv(opts: { legacyToken?: string; mappedTokens?: string; ttl?: string } = {}) {
  ...
  const env: StubEnv = {
    DEJA_SHARED_TOKEN: opts.legacyToken,
    DEJA_SHARED_TOKENS: opts.mappedTokens,
    DEJA_SHARED_STREAM_TTL_SECONDS: opts.ttl,
    MEMORY: {
...
describe("worker entry: local token-to-space routing", () => {
  ...
  test("mapped token routes to its named memory space and forwards that space", async () => {
    ...
    expect(seen.request?.headers.get("x-deja-space")).toBe("alice");
  });
  ...
  test("legacy single local token remains supported only in the local space", async () => {
    ...
    expect(seen.request?.headers.get("x-deja-space")).toBe("local");
  ...
  test("DEJA_SHARED_STREAM_TTL_SECONDS overrides the default TTL header", async () => {
