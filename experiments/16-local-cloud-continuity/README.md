...
> **Repository-scoped Deja owns the active local handoff. The shared Worker +
...
local client process                    cloud-shaped client process
  Deja DB (repo scope)                    separate Deja DB (same scope identity)
  SharedDeja mirror A                     SharedDeja mirror B
           │                                      │
...
with separate Deja and shared-mirror SQLite files. It sends synthetic documents
...
1. **Scope:** two synthetic Git checkouts with the same normalized origin derive
   the same Deja repository scope; an unrelated origin derives a different one.
2. **Immediate operational continuity:** the local process gets a local active
   ...
   exact slip and handoff through its own mirror. Its independent local Deja DB
...
- Shared handoffs currently have no shared resolve event. The cloud resolution
  ...
  lifecycle is resolved in Deja's repository-scoped handoff store.
