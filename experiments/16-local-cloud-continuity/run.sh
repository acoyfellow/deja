...
cleanup() {
  ...
  rm -f "$TMP"/*-mirror.sqlite* "$TMP"/*-deja.sqlite*
}
trap cleanup EXIT
...
(
  ...
  wrangler dev --local --ip 127.0.0.1 --port "$PORT" \
    --persist-to "$TMP/wrangler" --var "DEJA_SHARED_TOKEN:$TOKEN"
...
export DEJA_SHARED_GATEWAY="$BASE"
export DEJA_SHARED_TOKEN="$TOKEN"
