...
const GATEWAY = need("DEJA_SHARED_GATEWAY");
const TOKEN = need("DEJA_SHARED_TOKEN");
  ...
  constructor(role, cwd) {
    ...
    this.child = spawn("bun", ["run", resolve(HERE, "client.ts")], {
      ...
      env: {
        ...
        EXP16_LOCAL_DB: resolve(TMP, `${role}-deja.sqlite`),
        DEJA_AUTHOR: `exp16-${role}`,
        DEJA_SESSION: `exp16-${role}-${RUN}`,
      },
    });
...
async function resolveLocalAfterRestart(handoffId, query) {
  return await new Promise((resolvePromise, reject) => {
    const child = spawn("bun", ["run", resolve(HERE, "resolve-local.ts")], {
      ...
      env: {
        ...process.env,
        EXP16_LOCAL_DB: resolve(TMP, "local-deja.sqlite"),
        EXP16_HANDOFF_ID: handoffId,
        EXP16_QUERY: query,
        DEJA_AUTHOR: "exp16-local-resolver",
        DEJA_SESSION: `exp16-local-resolver-${RUN}`,
      },
    });
...
try {
  ...
  assert(cloudLocal.texts.length === 0 && cloudLocal.activeHandoff === null, "isolated local Deja DB unexpectedly transported state");
  ...
  // lifecycle is durable in its repository-scoped Deja DB, not process memory.
  local.child.kill();
  local = null;
  await delay(100);
