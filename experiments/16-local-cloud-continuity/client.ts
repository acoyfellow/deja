...
import { Deja, SharedDeja } from "../../src/index.ts";
import { currentMemoryContext } from "../../src/context.ts";

const role = need("EXP16_ROLE");
const shared = await SharedDeja.connect({
  gateway: need("DEJA_SHARED_GATEWAY"),
  token: need("DEJA_SHARED_TOKEN"),
  author: `exp16-${role}`,
  sessionId: `exp16-${role}-${need("EXP16_RUN")}`,
  mirrorOptions: { path: need("EXP16_MIRROR_DB") },
});
const local = new Deja({
