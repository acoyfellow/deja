#!/usr/bin/env bash
# Experiment 04: Can Deja be passed into a Terrarium child?
#
# This script:
#   1. Creates an isolated DEJA_DB under ./evidence/<timestamp>/
#   2. Exports DEJA_DB and DEJA_AUTHOR=parent-04
...
#
# Intentionally avoids global config changes. Does not edit ~/.deja,
...
# Isolated DB lives inside the experiment dir — never touches ~/.deja.
export DEJA_DB="$EVIDENCE_DIR/deja.db"
export DEJA_AUTHOR="parent-04"
export EXPERIMENT_EVIDENCE_DIR="$EVIDENCE_DIR"

REPO_ROOT="$(cd ../.. && pwd)"
export DEJA_REPO_ROOT="$REPO_ROOT"
DEJA_CLI=(bun run "$REPO_ROOT/src/cli.ts")

echo
echo "=== STEP 1: confirm isolated DB does not exist yet ==="
ls -l "$DEJA_DB" 2>&1 || echo "(expected: no such file)"

echo
echo "=== STEP 2: seed parent message into isolated DEJA_DB ==="
"${DEJA_CLI[@]}" send child-04 "ping from parent-04, run=$TS" \
  2>&1 | tee "$EVIDENCE_DIR/parent-send.txt"
PARENT_SEND_EXIT=${PIPESTATUS[0]}
echo "parent-send-exit=$PARENT_SEND_EXIT"

echo
echo "=== STEP 3: confirm isolated DB now exists & is NOT ~/.deja ==="
ls -l "$DEJA_DB"
"${DEJA_CLI[@]}" stats 2>&1 | tee "$EVIDENCE_DIR/parent-stats.txt"

echo
echo "=== STEP 4: spawn Terrarium child with deterministic agent ==="
AGENT="$(pwd)/agent/deterministic-agent.sh"
echo "agent=$AGENT"
echo "DEJA_DB=$DEJA_DB"
echo "DEJA_AUTHOR=$DEJA_AUTHOR"
...
# 5a. env-inheritance: child env file should exist & contain our DEJA_DB
...
assert "child saw DEJA_DB=$DEJA_DB" \
  "grep -qx \"DEJA_DB=$DEJA_DB\" \"$EVIDENCE_DIR/child-env.txt\"" \
  || FAILS=$((FAILS+1))
assert "child saw DEJA_AUTHOR=parent-04 (inherited)" \
  "grep -qx \"DEJA_AUTHOR=parent-04\" \"$EVIDENCE_DIR/child-env.txt\"" \
  || FAILS=$((FAILS+1))

# 5b. shared-DB write: query the SAME DB the parent created.
echo
echo "--- parent-04 inbox (should contain child-04's message) ---"
"${DEJA_CLI[@]}" inbox parent-04 --all 2>&1 | tee "$EVIDENCE_DIR/parent-inbox-after.txt"
...
# 5c. isolation: we did NOT touch ~/.deja
echo
echo "--- isolation check: ~/.deja mtimes ---"
ls -l "$HOME/.deja" 2>/dev/null | tee "$EVIDENCE_DIR/home-deja-listing.txt" || true
# Soft check — we can't fully prove no other process wrote to ~/.deja, but
# we can prove the slip we wrote went to the experiment DB by checking it
# exists in $DEJA_DB.
assert "isolated DB file exists at \$DEJA_DB" "[ -s \"$DEJA_DB\" ]" \
  || FAILS=$((FAILS+1))
