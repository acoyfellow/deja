...
Can a Terrarium-spawned child agent inherit Deja such that it reads and writes
to the same isolated Deja DB the parent is using?

## Hypothesis

H1. Terrarium spawns its child as a normal subprocess, so the child inherits
the parent shell environment. Therefore `DEJA_DB` and `DEJA_AUTHOR` exported
in the parent shell should be visible to the child.

H2. If the child agent command is `bun run src/cli.ts` (the Deja CLI itself,
not an LLM), the child can write a slip into the parent's isolated Deja DB.
This proves the *transport* — that Deja state can ride down through Terrarium
...
So "Deja reaches the child" is purely a function of (a) env inheritance and
...
- We do NOT test whether an LLM child would voluntarily call deja MCP.
- We do NOT mutate any user-global config (no `~/.terrarium/config.*` edits,
  no opencode config edits, no `~/.deja` writes — we point `DEJA_DB` at a
...
1. `run.sh` creates a temp dir under `experiments/04-.../evidence/` for this
   run. It exports `DEJA_DB=<temp>/deja.db` and `DEJA_AUTHOR=parent-04`.
...
4. The deterministic agent script is a plain bash script that:
   - Prints all `DEJA_*` env vars it sees (env-inheritance proof).
   - Re-invokes `bun run src/cli.ts` with a different `DEJA_AUTHOR` and writes
     a slip into the same `DEJA_DB`, proving the child can write Deja state
     into the parent-scoped DB.
   - Runs `deja ls` so the child can see the parent's earlier slip too
     (read-back proof).
...
- ✅ **Env inheritance proven** iff the Terrarium log for the child run
  contains `DEJA_DB=` and `DEJA_AUTHOR=` lines with the values the parent
  exported.
- ✅ **Shared-DB read/write proven** iff after the run, the isolated
  `deja.db` contains slips authored by both `parent-04` and `child-04`.
...
- That an LLM child agent (e.g., `opencode run`) would *autonomously* choose
  to call the deja MCP. That is a behavior/policy question and a different
  experiment.
- That Terrarium injects MCP config into the child. It does not. Whether the
  child agent has a Deja MCP wired in is a property of the child agent's own
