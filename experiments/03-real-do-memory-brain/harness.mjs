...
FTS5 status: **${ftsMode}**. ${
    ftsMode === "fts5"
      ? "Local DO SQL accepted `CREATE VIRTUAL TABLE ... USING fts5(...)` and answered a MATCH query. Real production Deja can lean on the same path."
      : `Local DO SQL rejected FTS5 at init time. The exact error was:\n\n\u0060\u0060\u0060\n${ftsError ?? "(no message)"}\n\u0060\u0060\u0060\n\nThe experiment fell back to a \`LIKE\`-based recall in the same code path, so the visibility/concurrency claims still hold — only the ranking model differs from production Deja.`
  }
