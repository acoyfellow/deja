...
Can a laptop agent and a remote agent select one **personal** Deja memory namespace from Cloudflare Access identity, without configuring a reusable API key in either agent?

Proposed request path:

```text
human SSO/WARP session -> short-lived Access assertion -> exact JWT validation
  -> SHA-256("deja/access-person/v1", verified issuer, verified sub)
...
- **Memory-content safety.** An authenticated agent can still write poisoned or sensitive memory. Deja's trust/provenance and content controls remain separate.
