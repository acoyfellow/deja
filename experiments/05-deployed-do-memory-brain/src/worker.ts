...
//   - `/reset` is exposed *only* when the `DEJA_EXP_ALLOW_RESET` env var
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    fwd.headers.set("x-deja-allow-reset", env.DEJA_EXP_ALLOW_RESET === "1" ? "1" : "0");
    fwd.headers.set("x-deja-build", env.DEJA_EXP_BUILD ?? "unknown");
...
export class BrainDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const allowReset = request.headers.get("x-deja-allow-reset") === "1";
    const build = request.headers.get("x-deja-build") ?? "unknown";
    ...
    if (url.pathname === "/reset" && request.method === "POST") {
      if (!allowReset) {
        return json(403, {
          ok: false,
          error:
            "reset disabled — set DEJA_EXP_ALLOW_RESET=1 in the worker env to enable",
        });
      }
      const before = (
