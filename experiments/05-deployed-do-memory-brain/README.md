...
`ctx.storage.sql` satisfies Deja's shared-memory contract under
`wrangler dev --local`:
...
**This experiment is the first in the Deja series that can mutate your
...
- creates / updates a Worker named `deja-exp05-deployed-do-memory-brain`
...
- Unique worker name `deja-exp05-deployed-do-memory-brain` — never
...
- `/reset` is exposed only when the worker was deployed with
  `DEJA_EXP_ALLOW_RESET=1` in its env (or `wrangler secret put` it).
...
- `wrangler.toml` — worker name `deja-exp05-deployed-do-memory-brain`,
...
wrangler secret put DEJA_EXP_ALLOW_RESET   # paste: 1
...
```bash
REMOTE_URL=https://deja-exp05-deployed-do-memory-brain.<your-subdomain>.workers.dev \
