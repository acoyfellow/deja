...
if [ -z "${REMOTE_URL:-}" ]; then
  ...
  echo "  REMOTE_URL=https://deja-exp05-deployed-do-memory-brain.<sub>.workers.dev \\"
...
if [ "${RESET:-0}" = "1" ]; then
  RESET_FLAG=(--reset)
  echo "[remote-harness.sh] /reset enabled. Worker must be deployed with DEJA_EXP_ALLOW_RESET=1."
fi
