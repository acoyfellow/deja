 ...
 * (DEJA_SHARED_MIRROR_DB). This proves real MCP JSON-RPC stdio behaviour and
...
const GATEWAY = required("DEJA_SHARED_GATEWAY");
const TOKEN = required("DEJA_SHARED_TOKEN");
...
async function spawnSharedMcp(label, mirrorDb) {
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      DEJA_SHARED_GATEWAY: GATEWAY,
      DEJA_SHARED_TOKEN: TOKEN,
      DEJA_SHARED_MIRROR_DB: mirrorDb,
      DEJA_AUTHOR: `exp10-${label}`,
      DEJA_SESSION: `exp10-${label}-session`,
    },
    stderr: "pipe",
  });
