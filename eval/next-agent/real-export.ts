...
const db = new Database(`${process.env.HOME}/.deja/deja.db`, { readonly: true });
...
for (const c of ['current','rules','ranker'] as const) {
  const selected = rank(slips, c, 'what should next agent know about current Deja work').slice(0,15);
