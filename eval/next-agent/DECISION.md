...
Do **not** add the old standalone `notice` / novelty feature to Deja.
...
- Current datasets are synthetic. Need one real Deja DB dogfood pass before integration.
- The feature should not be a new package yet. If added, it belongs as a small internal Deja ranking helper.
...
3. Run on a real Deja memory export.
4. If ranker still beats/ties rules-only while reducing noise, then consider a tiny Deja core helper.
