...
**All three runtimes now apply time-based decay on the read path.** No cron jobs, no manual scripts — decay is computed automatically when ranking memories for retrieval:
...
For the **hosted service**, this decay affects `inject()` ranking. Stored confidence still changes only when you explicitly call `confirm()` or `reject()`, or when cleanup removes low-confidence entries.
...
In all three runtimes, heavily rejected memories don't just fade away — they transform into **anti-patterns** that actively warn agents.
