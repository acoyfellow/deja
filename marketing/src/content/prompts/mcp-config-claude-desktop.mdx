...
This is the MCP server configuration for connecting Claude Desktop to your deja instance. Once configured, Claude can call deja's memory, working-state, and loop-run tools as native MCP tools. Secret CRUD is still HTTP-only.
...
Claude should list the deja tools, including `learn`, `confirm`, `reject`, `inject`, `query`, `forget`, `list`, and `stats`.
...
| `confirm` | Boost a memory's confidence | "Confirm memory abc-123" |
| `reject` | Lower confidence or invert to anti-pattern | "Reject memory abc-123" |
| `inject` | Get relevant memories | "What do you know about deploying?" |
| `inject_trace` | Inspect retrieval candidates and scores | "Why did this memory get injected?" |
| `query` | Search memories | "Search for anything about Node version issues" |
| `learning_neighbors` | Find similar memories | "Show related memories for abc-123" |
| `forget` | Delete a memory | "Forget memory abc-123" |
| `forget_bulk` | Bulk delete memories by filters | "Delete memories below 0.2 confidence" |
| `list` | List memories | "Show me all shared memories" |
| `stats` | Get statistics | "How many memories are stored?" |
| `state_put` / `state_get` / `state_patch` / `state_resolve` | Manage live working state | "Save the current run state" |
| `record_run` / `get_runs` | Track loop-run outcomes | "Record this run as a pass" |
