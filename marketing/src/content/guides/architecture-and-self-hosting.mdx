...
The SQLite schema includes a primary learnings table plus auxiliary tables for secrets, working state, state revisions/events, and loop runs. The memory table looks like this:
...
2. Vectorize is queried for nearby memories in the same scope
3. Near-identical matches are deduplicated; conflicting matches may be linked via `supersedes`
4. If a new row is needed, the resulting 384-dimensional vector is stored in Vectorize with the memory ID
5. The memory metadata is stored in SQLite
...
8. DO updates `last_recalled_at` and `recall_count` for the returned memories
9. Response returned as formatted prompt + raw learnings
```

Stored confidence changes only on explicit feedback (`confirm()` / `reject()`). Retrieval uses time-based decay for ranking so stale memories lose priority without mutating the stored value.
...
- `tools/list` — returns the available memory, state, and loop-run tools
- `tools/call` — executes a tool and returns results
- `notifications/initialized` and `notifications/cancelled` — handled as no-ops

MCP tools map to the same underlying memory/state operations as the REST API. Secret CRUD remains HTTP-only today.
...
Want to see how deja's architecture enables different memory interfaces? Our [Memory Interface Research](/research) explores 20 concepts — including the [Memory Debugger](/research/memory-debugger) for tracing request flows and the [Scope Telescope](/research/scope-telescope) for visualizing scope hierarchies.
